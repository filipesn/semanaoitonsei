...
```
ValorTotal = (valor recebido)
Se ValorTotal é < 50.00, exibir "Frete não disponível!"
Se ValorTotal é > ou = 200.00, exibir "Frete grátis!"
Se todas condições falirem, exibir "Frete com custo adicional!"
```
...
Classe Veiculo
    ...
    Método Construtor(modelo e ano)
        este modelo = modelo
        este ano = ano
...
Classe Carro herda Veiculo
    ...
    Método Construtor(modelo, ano e eficiencia)
        Chamar super(modelo e ano)
        esta eficiencia = eficiencia
...
```
def landtime(startspeed, safespeed, decel, maxtime):
    if decel <= 0:
        return "Desaceleração muito báixa"
    
    timeneeded = (startspeed - safespeed) / decel
    
    if timeneeded > maxtime:
        return "Não tem tempo suficiente"
